...
import { FUN_PRESETS } from "@/lib/presets/fun";
...
describe("fun preset set", () => {
  it("exports exactly the four playful presets, uniquely identified", () => {
    expect(FUN_PRESETS.map((p) => p.id)).toEqual(["memo", "arcade", "polaroid", "doodle"]);
    expect(new Set(FUN_PRESETS.map((p) => p.id)).size).toBe(4);
    for (const p of FUN_PRESETS) expect(p.category).toBe("playful");
  });

  it("declares a usable blurb, brief and slide range", () => {
    for (const p of FUN_PRESETS) {
...
describe.each(FUN_PRESETS.map((p) => [p.id, p] as const))("fun preset %s", (_id, preset) => {
